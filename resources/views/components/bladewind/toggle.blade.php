...
<label class="relative @if (!$justified)inline-flex @else flex justify-between @endif items-center group" onclick="{!! $onclick !!}">
    ...
    <input type="checkbox" name="{{ $name }}" @if ($checked) checked @endif @if ($disabled) disabled @endif class="absolute left-1/2 -translate-x-1/2 w-full h-full peer appearance-none rounded-md cursor-pointer" />
    <span style="zoom: 80%" class="w-20 @if ($bar=='thick')h-10 @else h-5 @endif flex items-center flex-shrink-0 p-1 bg-gray-300 rounded-full duration-300 ease-in-out cursor-pointer
