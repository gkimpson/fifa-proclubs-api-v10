...
<label style="zoom:95%" id="@if ($add_id_prefix)bw-@endif{{ $id }}" class="text-xs uppercase px-[10px] py-[5px] tracking-widest whitespace-nowrap inline-block rounded-md bg-{{ $color }}-{{ $color_weight[$shade] }} text-{{ $color }}-{{ $text_color_weight[$shade] }} {{ $class }}">
{{ $label }}
@if ($can_close)
